...
```
This function is too long and awkward to read.

Instead we can use tuple variables:
...
These tuple type annotations guarantee that the types fo the elements will be valud function parameters for ```gpsNavigate()```

Then, we can use JavaScript's spread syntax to write a more readable function call:
``` javascript
